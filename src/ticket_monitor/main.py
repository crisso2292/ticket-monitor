...
async def main() -> None:
    ...
    gametime_listings = await fetch_gametime(config)

    all_listings = gametime_listings

    if not gametime_listings:
    ...
    logger.info(
        "Cycle complete: %d Gametime listings fetched, %d alerts sent",
        len(all_listings),
        alerts_sent,
    )
